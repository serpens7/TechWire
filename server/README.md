...
| `npm run test:e2e` | API-тесты (133 шт.) — **очищает и пересевает БД** |
...
| GET | `/highlights` | тизеры для главной |
| GET | `/health` | проверка живости + доступности БД |

## Кто что может

Читать может любой, писать — только вошедшие.

**Открыто без токена:** `GET /articles`, `GET /articles/:id`, `GET /comments`,
`GET /highlights`, `POST /login`, `GET /health`.

**Требует входа:** всё, что создаёт или меняет содержимое — комментарий,
оценка, профиль, — плюс личные данные (`/notifications`, `/profile`,
`/article-ratings`: оценка привязана к пользователю, гостю там смотреть
нечего). Создание и правка статей дополнительно требуют роли `ADMIN`.

Технически: глобальный `JwtAuthGuard` закрывает всё, исключения помечаются
`@Public()`. Обратите внимание, что `@ApiBearerAuth()` висит на отдельных
методах, а не на классах контроллеров: на классе он пометил бы токеном и
публичные маршруты, и схема начала бы врать про доступ — `@Public()` влияет
на гвард, но не на документацию.

**`/highlights`** отдаёт статью дня и сниппет дня одним запросом; выбор
сниппета (одна ротация в день) делает сервер. Появился, когда каталог был
закрыт целиком и гостю показывали только тизеры; после открытия чтения
остался как способ не делать два запроса ради главной.
...
`npm run test:e2e` — 133 теста через supertest поверх настоящего Postgres, а не
моков: проверяется реальное поведение БД (уникальные индексы, каскады, upsert).

Покрыто: выдача и проверка токена, регистрация и занятость логина, лимит
попыток входа, закрытость маршрутов, RBAC, авторство из токена, валидация тел
и query, пагинация и фильтры, upsert рейтингов, подсчёт просмотров и окно
«статьи дня», публичная карточка автора, ветки комментариев и их удаление.
Отдельно закреплены найденные дефекты json-server, чтобы они не вернулись.
